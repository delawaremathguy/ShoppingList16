		...
		VStack(spacing: 0) {
			...
			List {
				Section("Shopping List Archive") {
					...
					Button {
						archiveDocument = ArchiveDocument(locations: Location.allLocations())
...
extension MoreView {
	...
	func loadSampleData() {
		...
		let locationRepresentations: [LocationRepresentation] = Bundle.main.decode(from: "ExportedShoppingList.json")

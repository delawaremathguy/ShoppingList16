...
edited values.  however, because we have to intercept when the user taps
the Back button, we'll use our own Back button.  (we don't really need
...
struct ModifyExistingItemView: View {
	...
	var body: some View {
		...
		// really look all that good.  it has to be localized, but it should at least point
		// in the right direction for all languages because of the use of the
		// SFSymbol "chevron.backward."
		// (i do not know of a way to detect when the user taps the
		// system-generated Back button, and that's why there's a custom button.)

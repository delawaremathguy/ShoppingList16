...
its quantity_ attribute smooths the transition between Int32 and Int.
...
the only property of Item that is not "underscored" is the `id` property,
which is a UUID.  in iOS13, this was useful as the back end of Identifiable;
but by iOS14, Core Data had syntesthized the `id` property itself,
although continuing to use the original `id` property defined as a
Core Data attribute was no problem.  going forward, it would be nice
to rename the `id` property, but with data already established in the cloud,
you cannot rename the property.  but in SwiftData in iOS 17, this will be
an issue (and that's one of the reasons we now have an Archive facility
to offload data and then restore it ...
